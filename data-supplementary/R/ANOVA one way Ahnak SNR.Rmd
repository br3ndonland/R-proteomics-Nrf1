...
subtitle: One-way ANOVA for normalized total summed signal to noise ratio (SNR) of
  Ahnak in the TCMP Nrf1 proteomics mouse liver AP-MS dataset from March 16, 2016.
---
# import dataset
```{r}
...
```{r}
library(car)
leveneTest(Ahnak, orderedgroups)
```

# anova in lm for comparison
```{r}
lm1 <- lm(Ahnak~orderedgroups)
anova(lm1)
```
...
<!-- meanslog <- aggregate(log(Ahnak)~orderedgroups, ProteinData, mean, levels = c("lacZ","Nrf1-HA chow","Nrf1-HA chol","Nrf1-HA bort")) # recalculate means for log transformed data -->
<!-- meanslog # verify mean values and ordering -->

<!-- aovlog <- aov(log(Ahnak)~orderedgroups) -->
<!-- summary(aovlog) -->
<!-- ``` -->
...
<!-- ```{r} -->
<!-- lmlog <- lm(log(Ahnak)~orderedgroups) -->
<!-- anova(lmlog) -->
<!-- ``` -->
...
<!-- outlierTest(lmlog) -->
<!-- leveneTest(log(Ahnak), orderedgroups) -->
<!-- ``` -->