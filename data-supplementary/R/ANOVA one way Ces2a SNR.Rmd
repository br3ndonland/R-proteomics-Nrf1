...
subtitle: One-way ANOVA for normalized total summed signal to noise ratio (SNR) of
  Ces2a in the TCMP Nrf1 proteomics mouse liver AP-MS dataset from March 16, 2016.
---
# import dataset
```{r}
...
```{r}
library(car)
leveneTest(Ces2a, orderedgroups)
```

# anova in lm for comparison
```{r}
lm1 <- lm(Ces2a~orderedgroups)
anova(lm1)
```
...
<!-- meanslog <- aggregate(log(Ces2a)~orderedgroups, ProteinData, mean, levels = c("lacZ","Nrf1-HA chow","Nrf1-HA chol","Nrf1-HA bort")) # recalculate means for log transformed data -->
<!-- meanslog # verify mean values and ordering -->

<!-- aovlog <- aov(log(Ces2a)~orderedgroups) -->
<!-- summary(aovlog) -->
<!-- ``` -->
...
<!-- ```{r} -->
<!-- lmlog <- lm(log(Ces2a)~orderedgroups) -->
<!-- anova(lmlog) -->
<!-- ``` -->
...
<!-- outlierTest(lmlog) -->
<!-- leveneTest(log(Ces2a), orderedgroups) -->
<!-- ``` -->