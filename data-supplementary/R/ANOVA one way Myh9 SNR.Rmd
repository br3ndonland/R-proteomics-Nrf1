...
subtitle: One-way ANOVA for normalized total summed signal to noise ratio (SNR) of
  Myh9 in the TCMP Nrf1 proteomics mouse liver AP-MS dataset from March 16, 2016.
---
# import dataset
```{r}
...
```{r}
library(car)
leveneTest(Myh9, orderedgroups)
```

# anova in lm for comparison
```{r}
lm1 <- lm(Myh9~orderedgroups)
anova(lm1)
```
...
<!-- meanslog <- aggregate(log(Myh9)~orderedgroups, ProteinData, mean, levels = c("lacZ","Nrf1-HA chow","Nrf1-HA chol","Nrf1-HA bort")) # recalculate means for log transformed data -->
<!-- meanslog # verify mean values and ordering -->

<!-- aovlog <- aov(log(Myh9)~orderedgroups) -->
<!-- summary(aovlog) -->
<!-- ``` -->
...
<!-- ```{r} -->
<!-- lmlog <- lm(log(Myh9)~orderedgroups) -->
<!-- anova(lmlog) -->
<!-- ``` -->
...
<!-- outlierTest(lmlog) -->
<!-- leveneTest(log(Myh9), orderedgroups) -->
<!-- ``` -->