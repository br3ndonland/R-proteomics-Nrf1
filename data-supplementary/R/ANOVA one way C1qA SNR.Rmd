...
output:
  html_document:
    ...
    theme: readable
    toc: yes
subtitle: One-way ANOVA for normalized total summed signal to noise ratio (SNR) of C1qA in the TCMP Nrf1 proteomics mouse liver AP-MS dataset from March 16, 2016.
---
# import dataset
```{r}
...
```{r}
library(car)
leveneTest(C1qA, orderedgroups)
```

# anova in lm for comparison
```{r}
lm1 <- lm(C1qA~orderedgroups)
anova(lm1)
```
...
<!-- meanslog <- aggregate(log(C1qA)~orderedgroups, ProteinData, mean, levels = c("lacZ","Nrf1-HA chow","Nrf1-HA chol","Nrf1-HA bort")) # recalculate means for log transformed data -->
<!-- meanslog # verify mean values and ordering -->

<!-- aovlog <- aov(log(C1qA)~orderedgroups) -->
<!-- summary(aovlog) -->
<!-- ``` -->
...
<!-- ```{r} -->
<!-- lmlog <- lm(log(C1qA)~orderedgroups) -->
<!-- anova(lmlog) -->
<!-- ``` -->
...
<!-- outlierTest(lmlog) -->
<!-- leveneTest(log(C1qA), orderedgroups) -->
<!-- ``` -->