...
subtitle: One-way ANOVA for normalized total summed signal to noise ratio (SNR) of
  Pdia3 in the TCMP Nrf1 proteomics mouse liver AP-MS dataset from March 16, 2016.
---
# import dataset
```{r}
...
```{r}
library(car)
leveneTest(Pdia3, orderedgroups)
```

# anova in lm for comparison
```{r}
lm1 <- lm(Pdia3~orderedgroups)
anova(lm1)
```
...
<!-- meanslog <- aggregate(log(Pdia3)~orderedgroups, ProteinData, mean, levels = c("lacZ","Nrf1-HA chow","Nrf1-HA chol","Nrf1-HA bort")) # recalculate means for log transformed data -->
<!-- meanslog # verify mean values and ordering -->

<!-- aovlog <- aov(log(Pdia3)~orderedgroups) -->
<!-- summary(aovlog) -->
<!-- ``` -->
...
<!-- ```{r} -->
<!-- lmlog <- lm(log(Pdia3)~orderedgroups) -->
<!-- anova(lmlog) -->
<!-- ``` -->
...
<!-- outlierTest(lmlog) -->
<!-- leveneTest(log(Pdia3), orderedgroups) -->
<!-- ``` -->