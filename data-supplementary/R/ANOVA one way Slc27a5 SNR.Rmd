...
subtitle: One-way ANOVA for normalized total summed signal to noise ratio (SNR) of
  Slc27a5 in the TCMP Nrf1 proteomics mouse liver AP-MS dataset from March 16, 2016.
---
# import dataset
```{r}
...
# anova in aov
```{r}
aov1 <- aov(Slc27a5~orderedgroups)
summary(aov1)
```
...
```{r}
library(car)
leveneTest(Slc27a5, orderedgroups)
```

# anova in lm for comparison
```{r}
lm1 <- lm(Slc27a5~orderedgroups)
anova(lm1)
```
...
<!-- meanslog <- aggregate(log(Slc27a5)~orderedgroups, ProteinData, mean, levels = c("lacZ","Nrf1-HA chow","Nrf1-HA chol","Nrf1-HA bort")) # recalculate means for log transformed data -->
<!-- meanslog # verify mean values and ordering -->

<!-- aovlog <- aov(log(Slc27a5)~orderedgroups) -->
<!-- summary(aovlog) -->
<!-- ``` -->
...
<!-- ```{r} -->
<!-- lmlog <- lm(log(Slc27a5)~orderedgroups) -->
<!-- anova(lmlog) -->
<!-- ``` -->
...
<!-- outlierTest(lmlog) -->
<!-- leveneTest(log(Slc27a5), orderedgroups) -->
<!-- ``` -->