...
output:
  html_document:
    ...
    theme: readable
    toc: yes
subtitle: One-way ANOVA for normalized total summed signal to noise ratio (SNR) of Cyp1a2 in the TCMP Nrf1 proteomics mouse liver AP-MS dataset from March 16, 2016.
---
# import dataset
```{r}
...
```{r}
library(car)
leveneTest(Cyp1a2, orderedgroups)
```

# anova in lm for comparison
```{r}
lm1 <- lm(Cyp1a2~orderedgroups)
anova(lm1)
```
...
<!-- meanslog <- aggregate(log(Cyp1a2)~orderedgroups, ProteinData, mean, levels = c("lacZ","Nrf1-HA chow","Nrf1-HA chol","Nrf1-HA bort")) # recalculate means for log transformed data -->
<!-- meanslog # verify mean values and ordering -->

<!-- aovlog <- aov(log(Cyp1a2)~orderedgroups) -->
<!-- summary(aovlog) -->
<!-- ``` -->
...
<!-- ```{r} -->
<!-- lmlog <- lm(log(Cyp1a2)~orderedgroups) -->
<!-- anova(lmlog) -->
<!-- ``` -->
...
<!-- outlierTest(lmlog) -->
<!-- leveneTest(log(Cyp1a2), orderedgroups) -->
<!-- ``` -->