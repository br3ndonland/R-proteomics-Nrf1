...
subtitle: One-way ANOVA for normalized total summed signal to noise ratio (SNR) of
  Cyp2c37 in the TCMP Nrf1 proteomics mouse liver AP-MS dataset from March 16, 2016.
---
# import dataset
```{r}
...
# anova in aov
```{r}
aov1 <- aov(Cyp2c37~orderedgroups)
summary(aov1)
```
...
```{r}
library(car)
leveneTest(Cyp2c37, orderedgroups)
```

# anova in lm for comparison
```{r}
lm1 <- lm(Cyp2c37~orderedgroups)
anova(lm1)
```
...
<!-- meanslog <- aggregate(log(Cyp2c37)~orderedgroups, ProteinData, mean, levels = c("lacZ","Nrf1-HA chow","Nrf1-HA chol","Nrf1-HA bort")) # recalculate means for log transformed data -->
<!-- meanslog # verify mean values and ordering -->

<!-- aovlog <- aov(log(Cyp2c37)~orderedgroups) -->
<!-- summary(aovlog) -->
<!-- ``` -->
...
<!-- ```{r} -->
<!-- lmlog <- lm(log(Cyp2c37)~orderedgroups) -->
<!-- anova(lmlog) -->
<!-- ``` -->
...
<!-- outlierTest(lmlog) -->
<!-- leveneTest(log(Cyp2c37), orderedgroups) -->
<!-- ``` -->